...
### Added
- Tiered chapter detection: marked chapters → heading detection → formatting fallback
- HeadingDetector for PDF/text files (known section names, isolated title lines, ALL CAPS fallback)
- Spoiler-protected end preview in auto-strip (hidden by default)
- Front-matter bias: copyright/title pages in first 20% flagged more aggressively
- Conservative back-stripping via `back_penalty` parameter

### Changed
- Strip command now detects real sections in PDFs instead of showing page numbers
- `find_content_boundaries()` accepts `back_penalty` and `front_bias` parameters
